...
interface ILicenseOCSPSetupProps {
  onCancel?: () => void;
}

interface ILicenseOCSPSetupState {
  license: string;
}

class LicenseOCSPSetup extends React.Component<ILicenseOCSPSetupProps, ILicenseOCSPSetupState> {
  static contextTypes = {
    locale: PropTypes.string,
    localize: PropTypes.func,
  };

  constructor(props: ILicenseOCSPSetupProps) {
    super(props);
  ...
  handleApplyLicense = () => {
    ...
    if (OS_TYPE === "Darwin") {
      ocspUtilPath = "/Applications/CryptoPro_ECP.app/Contents/MacOS/bin/ocsputil";
    } else {
      ocspUtilPath = os.arch() === "ia32" ? "/opt/cprocsp/bin/ia32/ocsputil" : "/opt/cprocsp/bin/amd64/ocsputil";
    }

    if (OS_TYPE === "Windows_NT") {
      if (os.arch() === "ia32") {
        // tslint:disable-next-line:max-line-length
        cmnd = 'reg add "HKLM\\SOFTWARE\\Crypto Pro\\OCSPAPI\\2.0" /v ProductID /t REG_SZ /f /d ' + license;
      } else {
        // tslint:disable-next-line:max-line-length
        cmnd = 'reg add "HKLM\\SOFTWARE\\WOW6432Node\\Crypto Pro\\OCSPAPI\\2.0" /v ProductID /t REG_SZ /f /d ' + license;
      }
    } else {
      // tslint:disable-next-line:quotemark
      cmnd = "sh -c " + "\"" + ocspUtilPath + ' license -s ' + "'" + license + "'" + "\"";
    }
...
export default LicenseOCSPSetup;
